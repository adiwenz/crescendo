...
def hz_to_note_and_cents(f0_hz):
    ...
    Includes:
      • smoothing of short jumps
      • outlier removal
    """
    midi_raw = librosa.hz_to_midi(f0_hz)

    # Smooth tiny jumps and drop outliers
    midi_smooth = smooth_short_jumps(midi_raw, max_len=2)
    midi_clean = remove_outlier_frames(midi_smooth, max_semitone_jump=4, window=4)
...
def plot_results(times, midi_float, midi_nearest, cents_note_avg, note_names):
    ...
    Interactive Melodyne-style plot:
      ...
      • sung pitch = blue line, but NOT connected across different notes
