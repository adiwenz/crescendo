...
import 'find_range_highest_screen.dart';

class FindRangeLowestScreen extends StatefulWidget {
  const FindRangeLowestScreen({super.key});

  @override
  State<FindRangeLowestScreen> createState() => _FindRangeLowestScreenState();
}

class _FindRangeLowestScreenState extends State<FindRangeLowestScreen> {
  ...
  String _status = 'Tap Start Listening and sing your lowest comfortable note.';
  ...
  void _onStableCaptured(int midi, String noteName) {
    ...
    Future.delayed(const Duration(seconds: 2), () async {
      ...
      final result = await Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => FindRangeHighestScreen(lowestMidi: midi)),
      );
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    final noteLabel = _stableMidi != null
        ? 'Lowest: ${PitchMath.midiToName(_stableMidi!)}'
        : 'Lowest: —';
    return Scaffold(
      appBar: AppBar(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_showSummary) ...[
              ...
              Text('Your lowest note was ${PitchMath.midiToName(_stableMidi!)}',
