...
def average_cents_per_note(midi_nearest, cents_off):
    ...
    while i < n:
        ...
        # Find end of this contiguous note region
        j = i + 1
        while (
            j < n
            and not np.isnan(midi_nearest[j])
            and midi_nearest[j] == midi_nearest[i]
        ):
            j += 1
        ...
        if np.any(valid):
            ...
            cents_avg[seg_slice] = mean_cents   # same average for all frames
...
def plot_results(times, midi_float, midi_nearest, cents_note_avg, note_names):
    ...
    # Actual sung pitch (scatter) – blue
    scatter = ax.scatter(
        times_v,
        midi_v,
        s=10,
...
def main():
    ...
    # 5) Print a few sample frames (still frame-level view)
