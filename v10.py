...
def plot_results(times, midi_float, midi_nearest, cents_note_avg, note_names):
    ...
    Melodyne-ish plot:
      ...
      • nearest note = orange horizontal segments (one per note)
    ...
    # --- Build nearest-note line: separate segment for EACH note ---
    nearest_line = np.full_like(midi_nearest_v, np.nan, dtype=float)
    n = len(midi_nearest_v)
    i = 0
    while i < n:
        if np.isnan(midi_nearest_v[i]):
            i += 1
            continue
        note = midi_nearest_v[i]
        start = i
        j = i + 1
        while j < n and not np.isnan(midi_nearest_v[j]) and midi_nearest_v[j] == note:
            j += 1

        # horizontal segment from start..j-1
        nearest_line[start:j] = note
        # break at boundaries so there is no connecting diagonal
    ...
    # --- Sung pitch line, broken between different nearest notes ---
    ...
    # Interactive hover: per-note-averaged cents
