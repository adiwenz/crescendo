...
def plot_results(times, midi_float, midi_nearest, cents_note_avg, note_names):
    ...
    Interactive Melodyne-style plot:
      ...
      • nearest note = orange LINE (flat per note, one value per note segment)
    ...
    # --- Nearest note: orange line ---
    # Because we already cleaned & segmented nearest MIDI, and set NaNs in gaps,
    # just plotting it directly will give flat lines per note and gaps in between.
    ax.plot(
        times_v,
        midi_nearest_v,
        "-",
    ...
    # Invisible scatter just for hover picking (so hover stays dense)
